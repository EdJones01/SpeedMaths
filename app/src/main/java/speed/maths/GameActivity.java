...
public class GameActivity extends Activity {
    ...
    public void checkAnswer() {
        recognizer.recognize(drawingView.build())
                .addOnSuccessListener(result -> {
                    if (getPossibleInts(result.getCandidates()).contains(getCurrentQuestion().getAnswer())) {
    ...
    LinkedList<Integer> getPossibleInts(List<RecognitionCandidate> candidates) {
        LinkedList<Integer> nums = new LinkedList<>();
        for (int i = 0; i < candidates.size(); i++) {
            try {
                nums.add(Integer.parseInt(candidates.get(i).getText()));
            } catch (Exception e) {
                continue;
            }
        }
